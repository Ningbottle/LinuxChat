...
#include <openssl/evp.h>  // modern EVP API (OpenSSL 1.1/3.x, no deprecation for SHA-256)
...
/// SHA-256 hash → lowercase hex string (OpenSSL 3.x compatible via EVP)
...
struct Config {
    int    port     = 8080;
    int    workers  = 4;
    std::string db_path = "linuxchat.db";  // relative to CWD; ensure writable dir. Use --db for absolute.
};
